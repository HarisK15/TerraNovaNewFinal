...
def generate_pandas_query(user_input, llm_model):
    ...
    prompt_template = ChatPromptTemplate.from_messages([
        ...
        - The query MUST use `df` as the dataframe name.
        - Simple counting/aggregation should NOT use groupby without group keys.
        - For simple counts, use `len(df)` or `df.shape[0]`.
        ...
        ✅ **Example 3**:
        User: "How many total customers are there?"
        Response:
        len(df)
        
        ✅ **Example 4**:
        User: "Count the number of orders"
        Response:
        df.shape[0]
        
        ✅ **Example 5**:
        User: "What's the average price of products?"
        Response:
        df['price'].mean()
    ...
    ai_text = re.sub(r'```python\s*|\s*```', '', ai_text)
    ai_text = re.sub(r'```\s*|\s*```', '', ai_text)
    
    # Remove any explanatory text
    pandas_query = ai_text.strip()
    
    # Basic validation - ensure it uses the df variable
    if not (pandas_query.startswith('df') or pandas_query.startswith('len(df)') or 
